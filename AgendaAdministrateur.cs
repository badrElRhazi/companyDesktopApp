...
namespace csse3
{
    public partial class AgendaAdministrateur : Form
    {
        public AgendaAdministrateur()
        {
            InitializeComponent();
        }
        ...
        private void AgendaAdministrateur_Load(object sender, EventArgs e)
        {
            using (SqlConnection con1 = new SqlConnection(Program.connection))
            {
                con1.Open();

                SqlCommand cmd = new SqlCommand("select A. ID_AGENDAEMPLOYE,E.Nom ,A.DATE,A.TITRE from EMPLOYE E inner join AGENDA_EMPLOYE A on A.ID_EMPLOYE=E.ID_EMPLOYE where E.POSTE='Agent Administratif'", con1);
                try
                {
        ...
        private void DGV_Agenda_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    // cb_Date.Text= DGV_Agenda.CurrentRow.Cells[].Value.ToString();
