...
namespace csse3
{
    public partial class AgandaResponsable : Form
    {
        public AgandaResponsable()
        {
            InitializeComponent();
        }
        ...
        private void AgandaResponsable_Load(object sender, EventArgs e)
        {
            using (SqlConnection con1 = new SqlConnection(Program.connection))
            {
                con1.Open();

                SqlCommand cmd = new SqlCommand("select A. ID_AGENDAEMPLOYE,E.Nom ,A.DATE,A.TITRE from EMPLOYE E inner join AGENDA_EMPLOYE A on A.ID_EMPLOYE=E.ID_EMPLOYE where E.POSTE='Responsable Formation'", con1);
                try
                {
        ...
        private void DGV_Agenda_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 4)
            {
                using (SqlConnection con1 = new SqlConnection(Program.connection))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("select *from EMPLOYE where POSTE='Responsable Formation'", con1);
                    try
                    {
